...
struct Gorusme{
    ...
    char bas_zamani[5];
    int sure;
    struct Gorusme *tarihe_gore_sonraki;
    struct Gorusme *tarihe_gore_onceki;
};
...
void bir_aboneye_ait_son_3_gorusme(abone *);
...
int main()
{
    char harf,soyad_parca[13],abone_tip[9],tarih[9],cevap;
    int tel_no,hash_tel,hash_isim,i,secim;
    ...
    for(i=0;i<26;i++)//dairesel listenin ilk dugumleri ilkleniyor..
        ada_gore_hash_tablosu[i].soyada_gore_sonraki=&ada_gore_hash_tablosu[i];
        ...
        switch(secim){
            case 1:
                ...
                if(tele_gore_arama(telefona_gore_hash_tablosu[hash_tel],tel_no)==NULL){//Girilen numarada abone kayitli degilse islemler yapiliyor..
                    ...
                    yeni_abone->gorusmePtr->tarihe_gore_sonraki=yeni_abone->gorusmePtr;
                    yeni_abone->gorusmePtr->tarihe_gore_onceki=yeni_abone->gorusmePtr;
                    ...
                    printf("Abone basariyla eklenmistir!\n");
                }
                break;
            case 2:
            ...
            case 6:
                ...
                if(yeni_abone!=NULL){//Bu numarali abone varsa bilgisi ekrana yazdiriliyor ve gorusmeleri icin fonksiyon cagriliyor..
                    ...
                    printf("------- ---------- ------------ ------------------------- ----------\n");
                    printf("%-7d %-10s %-12s %-25s %-10s\n",tel_no,yeni_abone->ad,yeni_abone->soyad,yeni_abone->adres,abone_tip);
                    bir_aboneye_ait_son_3_gorusme(yeni_abone);
                }
                else printf("Bu numaraya kayitli bir abone yok!\n");
                break;

            case 7:
                printf("Telefon numaralarinin ilk uc hanesini giriniz:\n");
                scanf("%d",&tel_no);
...
void menu()
{
    ...
    printf("2) Bir abonenin nakil edilmesi \n");
    ...
    printf("6) Bir abonenin bilgilerinin ve yapmis oldugu son 3 gorusmelerin listelenmesi \n");
    printf("7) Telefon numarasi belirli bir 3 basamakli sayi ile baslayan abonelerin bilgilerinin listelenmesi \n");
    printf("8) Soyadi belirli bir harf ile baslayan abonelerin bilgilerinin listelenmesi\n");
    printf("9) Soyadi belirli bir metin parcasi ile baslayan abonelerin bilgilerinin listelenmesi \n");
...
int sayi_al(int alt_sinir, int ust_sinir)
{
    ...
    {
        printf("Yanlis giris yaptiniz. Lutfen %d-%d arasinda bir sayi giriniz. :",alt_sinir, ust_sinir);
        scanf("%d", &sayi);
    }
...
void tele_gore_sirali_ekleme(abone **ilkPtr,abone *yeni_abone)
{
    ...
    while(suanki!=NULL&&yeni_abone->tel_no>suanki->tel_no){
        onceki=suanki;
        suanki=suanki->tel_noya_gore_sonraki;
    }
    if(onceki==NULL){
        yeni_abone->tel_noya_gore_sonraki= *ilkPtr;
        *ilkPtr=yeni_abone;
    }
    else{
        onceki->tel_noya_gore_sonraki=yeni_abone;
        yeni_abone->tel_noya_gore_sonraki=suanki;
    }
}
void soyadina_gore_sirali_ekleme(abone *ilkPtr,abone *yeni_abone)
{
    ...
    onceki=ilkPtr;
    suanki=ilkPtr->soyada_gore_sonraki;
    while(suanki!=ilkPtr&&strncmp(strlwr(yeni_abone->soyad),strlwr(suanki->soyad),strlen(yeni_abone->soyad))>0){
        onceki=suanki;
        suanki=suanki->soyada_gore_sonraki;
    }
    yeni_abone->soyada_gore_sonraki=suanki;
...
void tele_gore_silme(abone **ilkPtr, abone *cikartilacak)
{
    ...
    if(onceki==cikartilacak){
        *ilkPtr=(*ilkPtr)->tel_noya_gore_sonraki;
        free(onceki);
    }
    else{
        suanki=(*ilkPtr)->tel_noya_gore_sonraki;
        while(suanki!=cikartilacak){
            onceki=suanki;
            suanki=suanki->tel_noya_gore_sonraki;
        }
        onceki->tel_noya_gore_sonraki=suanki->tel_noya_gore_sonraki;
        free(suanki);
    }
}
...
void gorusme_ekle(abone *abonePtr)
{
    ...
    if(abonePtr->gorusmePtr==gecici->tarihe_gore_sonraki){//Listeye ilk gorusme ekleniyor..
        gecici->tarihe_gore_onceki=yeni_gorusme;
        gecici->tarihe_gore_sonraki=yeni_gorusme;
        yeni_gorusme->tarihe_gore_sonraki=gecici;
        yeni_gorusme->tarihe_gore_onceki=gecici;
    }
    else{//Listede baska gorusmeler varsa tarihe gore sirali bir sekilde ekleme yapiliyor...
        gecici=gecici->tarihe_gore_sonraki;
        ...
        yeni_gorusme->tarihe_gore_sonraki=gecici;
        yeni_gorusme->tarihe_gore_onceki=gecici->tarihe_gore_onceki;
        gecici->tarihe_gore_onceki->tarihe_gore_sonraki=yeni_gorusme;
        gecici->tarihe_gore_onceki=yeni_gorusme;
    }
}
...
{//Silinecek abonenin gorusmeleride sirasi ile free yapiliyor..
    ...
    if(onceki==onceki->tarihe_gore_sonraki)//Gorusme yoksa baslangic dugumu free yapiliyor..
        free(onceki);
    else{
        suanki=onceki->tarihe_gore_sonraki;
        while(suanki!=abonePtr->gorusmePtr){//listenini hepsi baslangic dugumu haric free yapiliyor..
            free(onceki);
            onceki=suanki;
            suanki=suanki->tarihe_gore_sonraki;
        }
...
void ada_gore_abone_silme(abone *ada_gore_bas_dugumu,int tel_no)
{
    ...
    suanki=ada_gore_bas_dugumu->soyada_gore_sonraki;
    //listede birtek aradigimiz abone varsa silme islemi gerceklesiyor..
    if(suanki->soyada_gore_sonraki==ada_gore_bas_dugumu){
        ada_gore_bas_dugumu->soyada_gore_sonraki=ada_gore_bas_dugumu;
        free(suanki);
    }
        ...
        while(suanki!=ada_gore_bas_dugumu){
            if(tel_no==suanki->tel_no){//silinicek abone telefon numarasina gore bulunduktan sonra free yapiliyor..
                onceki->soyada_gore_sonraki=suanki->soyada_gore_sonraki;
                free(suanki);
                break;
            }
            onceki=suanki;
...
void bir_aboneye_ait_gorusmelerin_listesi(abone *abonePtr)
{
    ...
    while(gecici!=abonePtr->gorusmePtr){
        ...
        printf("%.2d.%.2d.%.2d %-13s %c%c:%c%c       %d\n",gorusme_gunu,gorusme_ayi,gorusme_yili,gecici->aranan_no
               ,gecici->bas_zamani[0],gecici->bas_zamani[1],gecici->bas_zamani[2],gecici->bas_zamani[3],gecici->sure);
        top_gorusme_sayisi++;
        top_gorusme_suresi+=gecici->sure;
        gecici=gecici->tarihe_gore_sonraki;
        gorusmenin_tarihinin_cozumlemesi(&gorusme_yili,&gorusme_ayi,&gorusme_gunu,gecici->tarih);
    }
    if(kontrol==1){
...
void bir_aboneye_ait_son_3_gorusme(abone *abonePtr)
{
    int gorusme_yili,gorusme_ayi,gorusme_gunu,kontrol=0,i;
    gorusme *gecici;

    gecici=abonePtr->gorusmePtr;//o abonenin gorusmelerinin baslangic dugumunun adresi gonderiliyor..
    gecici=gecici->tarihe_gore_onceki;
    gorusmenin_tarihinin_cozumlemesi(&gorusme_yili,&gorusme_ayi,&gorusme_gunu,gecici->tarih);
    for(i=0;i<3;i++)
    {
        if(kontrol==0){//Bir kere girdikten sonra bir daha bu ekrana yazdirilmiyor..
        ...
        if(gecici==abonePtr->gorusmePtr)//3 gorusmeden az gorusme sayisi varsa sona geldiginde fonksiyondan cikar
        {
            break;
        }
        printf("%.2d.%.2d.%.2d %-13s %c%c:%c%c       %d\n",gorusme_gunu,gorusme_ayi,gorusme_yili,gecici->aranan_no
               ,gecici->bas_zamani[0],gecici->bas_zamani[1],gecici->bas_zamani[2],gecici->bas_zamani[3],gecici->sure);
        gecici=gecici->tarihe_gore_onceki;//Yeniden eskiye
        gorusmenin_tarihinin_cozumlemesi(&gorusme_yili,&gorusme_ayi,&gorusme_gunu,gecici->tarih);
    }
...
void soyadin_bas_harfine_gore_ekrana_yazdirma(abone *ilk_dugum)
{
    ...
    suanki=ilk_dugum->soyada_gore_sonraki;
...
void abonelerin_gorusme_analizi(abone *abonePtr,int top_gorusme_sayisi[5],int top_gorusme_suresi[5])
{
    ...
    while(gecici!=abonePtr->gorusmePtr){
        if(abonePtr->abone_tipi=='K'){
            ...
            top_gorusme_suresi[0]+=gecici->sure;
        }
        else if(abonePtr->abone_tipi=='G'){
            top_gorusme_sayisi[1]++;
            top_gorusme_suresi[1]+=gecici->sure;
        }
        else if(abonePtr->abone_tipi=='I'){
            top_gorusme_sayisi[2]++;
            top_gorusme_suresi[2]+=gecici->sure;
        }
        else{
            top_gorusme_sayisi[3]++;
            top_gorusme_suresi[3]+=gecici->sure;
        }
        top_gorusme_sayisi[4]++;
        top_gorusme_suresi[4]+=gecici->sure;
        gecici=gecici->tarihe_gore_onceki;
    }
    ...
    for(i=0;i<900;i++){
        if(telefona_gore_hash_tablosu[i]!=NULL){//Bu listede abone varsa islemler yapiliyor..
            ...
            while (gecici!=NULL){
                ...
                gecici_gorusme=gecici_gorusme->tarihe_gore_onceki;
...
void bir_numarayi_arayan_kullanicilarin_listesi(abone ada_gore_hash_tablosu[26],int tel_no)
{
    ...
    for(i=0;i<26;i++){
        gecici=(&ada_gore_hash_tablosu[i])->soyada_gore_sonraki;
            while (gecici!=&ada_gore_hash_tablosu[i]){
                gecici_gorusme=gecici->gorusmePtr;
                gecici_gorusme=gecici_gorusme->tarihe_gore_sonraki;
                while(gecici_gorusme!=gecici->gorusmePtr){
                    if(atoi(gecici_gorusme->aranan_no)==tel_no){
                        kontrol=1;
                        break;
                    }
                    gecici_gorusme=gecici_gorusme->tarihe_gore_sonraki;
                ...
                if(kontrol==1){
                    if(kontrol_2==0&&ada_gore_hash_tablosu[i].soyada_gore_sonraki!=&ada_gore_hash_tablosu[i]){
                        printf("%-7s %-10s %-12s %-25s %-10s\n","Tel No","Adi","Soyadi","Adresi","Abone Tipi");
                        printf("------- ---------- ------------ ------------------------- ----------\n");
                        kontrol_2=1;
                    }
                ...
                while(gecici_gorusme!=gecici->gorusmePtr){
                    ...
                    if(gecici_gorusme_yili==gorusme_yili&&gecici_gorusme_ayi==gorusme_ayi&&gecici_gorusme_gunu==gorusme_gunu){
                        ...
                        printf("%-13d %.2d.%.2d.%.2d %-13s %c%c:%c%c       %d\n",gecici->tel_no,gorusme_gunu,gorusme_ayi,gorusme_yili,gecici_gorusme->aranan_no
                               ,gecici_gorusme->bas_zamani[0],gecici_gorusme->bas_zamani[1],gecici_gorusme->bas_zamani[2],gecici_gorusme->bas_zamani[3],gecici_gorusme->sure);
                    }
                    gecici_gorusme=gecici_gorusme->tarihe_gore_onceki;
                }
                gecici=gecici->tel_noya_gore_sonraki;
